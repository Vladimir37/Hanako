...
var NTLMResponsePayload = (function () {
  ...
  _createClass(NTLMResponsePayload, [{
    ...
    value: function createResponse(challenge) {
      ...
      var data = new _trackingBufferWritableTrackingBuffer2['default'](bufferLength);
      data.position = 0;
    ...
    value: function createTimestamp(time) {
      var tenthsOfAMicrosecond = new _bigNumber.n(time).plus(11644473600).multiply(10000000);
      var hexArray = [];
    ...
    value: function ntHash(text) {
      ...
      var md4 = _crypto2['default'].createHash('md4').update(unicodeString).digest();
      if (md4.copy) {
        md4.copy(hash);
      } else {
    ...
    value: function hmacMD5(data, key) {
      var hmac = _crypto2['default'].createHmac('MD5', key);
      hmac.update(data);
