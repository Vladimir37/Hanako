...
var _trackingBufferWritableTrackingBuffer = require('./tracking-buffer/writable-tracking-buffer');

var _trackingBufferWritableTrackingBuffer2 = _interopRequireDefault(_trackingBufferWritableTrackingBuffer);
...
var Login7Payload = (function () {
  function Login7Payload(loginData) {
    ...
    var data = new _trackingBufferWritableTrackingBuffer2['default'](300);
  ...
  _createClass(Login7Payload, [{
    ...
    value: function createFixedData() {
      this.tdsVersion = _tdsVersions.versions[this.loginData.tdsVersion];
      ...
      var buffer = new _trackingBufferWritableTrackingBuffer2['default'](100);
    ...
    value: function createVariableData(offset) {
      ...
      var variableData = {
        offsetsAndLengths: new _trackingBufferWritableTrackingBuffer2['default'](200),
        data: new _trackingBufferWritableTrackingBuffer2['default'](200, 'ucs2'),
      ...
      this.libraryName = _library.name;
    ...
    value: function createNTLMRequest(options) {
      ...
      var buffer = new _trackingBufferWritableTrackingBuffer2['default'](bufferLength);
    ...
    value: function toString(indent) {
      indent || (indent = '');
      return indent + 'Login7 - ' + (0, _sprintf.sprintf)('TDS:0x%08X, PacketSize:0x%08X, ClientProgVer:0x%08X, ClientPID:0x%08X, ConnectionID:0x%08X', this.tdsVersion, this.packetSize, this.clientProgVer, this.clientPid, this.connectionId) + '\n' + indent + '         ' + (0, _sprintf.sprintf)('Flags1:0x%02X, Flags2:0x%02X, TypeFlags:0x%02X, Flags3:0x%02X, ClientTimezone:%d, ClientLCID:0x%08X', this.flags1, this.flags2, this.typeFlags, this.flags3, this.clientTimeZone, this.clientLcid) + '\n' + indent + '         ' + (0, _sprintf.sprintf)("Hostname:'%s', Username:'%s', Password:'%s', AppName:'%s', ServerName:'%s', LibraryName:'%s'", this.hostname, this.loginData.userName, this.loginData.password, this.loginData.appName, this.loginData.serverName, _library.name) + '\n' + indent + '         ' + (0, _sprintf.sprintf)("Language:'%s', Database:'%s', SSPI:'%s', AttachDbFile:'%s', ChangePassword:'%s'", this.loginData.language, this.loginData.database, this.sspi, this.attachDbFile, this.changePassword);
    }
  }]);
